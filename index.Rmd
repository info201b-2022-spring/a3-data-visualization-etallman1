...
us_map$state <- str_replace_all(us_map$state, "vermont", "AL")
...
right_join(state_avg_simplified, by="state")
...
ggplot(us_map) +
   geom_polygon(
    mapping = aes(x = long, y = lat, group = group, fill = non_white_pop_total),
    color = "white", 
    size = .3        
  ) +
  coord_map() +
